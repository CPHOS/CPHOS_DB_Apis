...
for i in range(2, rowmax + 1):
    wxid = ws.cell(row=i, column=3).value
    ...
    wss.cell(i, 1).value = wxid
    wss.cell(i, 2).value = teacher_name
    wss.cell(i, 6).value = teacher_sup

    already_exist_sup_teacher = customTransaction.executeOperation(GetTeacherInfoByName(teacher_sup))
    if (len(already_exist_sup_teacher) == 0):
        wss.cell(i, 3).value = '上级领队不存在'
        wss.cell(i, 4).value = '上级领队不存在'
        continue
    elif (len(already_exist_sup_teacher) > 1):
        wss.cell(i, 3).value = '上级领队重名'
        wss.cell(i, 4).value = '上级领队重名'
        continue
    else:
        sup_teacher_dct = already_exist_sup_teacher[0]
        if sup_teacher_dct['p_id'] != 0:  # 这里0代表是领队 1代表不是
            wss.cell(i, 3).value = '上级领队不是领队'
            wss.cell(i, 4).value = '上级领队不是领队'
            continue
    ...
    if (len(already_exist_teacher) > 1):
        ...
        print("已经存在！现在我们把这位叫做{}的老师变成相应的副领队。".format(teacher_name))
    ...
    if not already_exist_flag:
        teacher_id_list = customTransaction.executeOperation(GetToBeVerifiedTeacherInfoByWechatName(wxid))
        if (len(teacher_id_list) == 0):
            print("故障：未找到微信名为{}，名称为{}的待审核教练".format(wxid, teacher_name))
            wss.cell(i, 3).value = '未找到'
            wss.cell(i, 4).value = '未提交审核'
            continue
        elif (len(teacher_id_list) > 1):
            wss.cell(i, 3).value = '故障'
            wss.cell(i, 4).value = '有重复的微信名或提交多份审核，需要人工操作'
            continue
