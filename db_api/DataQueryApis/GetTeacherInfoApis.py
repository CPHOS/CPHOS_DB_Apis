    ...
    def execute(self, cursor: pymysql.cursors.Cursor):
        try:
            ...
            for item in returned:
                returned_lst.append({'id': item[0], 'p_id': item[1], 'wechat_nickname': item[3], 'user_name': item[-8],
                                     'school_id': item[-7], 'upload_limit': item[-3], 'viewing_problem': item[-6],
                                     'type': typedict[item[-4]]})
            return returned_lst
...
class GetTeacherInfoByName(CustomOperation):
    ...
    def execute(self, cursor: pymysql.cursors.Cursor):
        try:
            ...
            returned = cursor.fetchall()
            returned_lst = []
            # print("GetTeacherInfoByName: ", returned)
            for item in returned:
                returned_lst.append({'id': item[0], 'p_id': item[1], 'wechat_nickname': item[3], 'user_name': item[-8],
                                     'school_id': item[-7], 'upload_limit': item[-3], 'viewing_problem': item[-6],
                                     'type': typedict[item[-4]]})
...
class GetToBeVerifiedTeacherInfoByWechatName(CustomOperation):
    ...
    def execute(self, cursor: pymysql.cursors.Cursor):
        try:
            ...
            returned_lst = []
            for item in returned:
                returned_lst.append({'id': item[0], 'wechat_nickname': item[3], 'user_name': item[-8]})
            return returned_lst
        except Exception as e:
            print(e)
            raise e
...
class GetToBeVerifiedTeacherInfoByFlexibleWechatName(CustomOperation):
    def __init__(self, WechatName: str):
        self.MySQLCommand = "select * from cmf_tp_member where `nickname` like \'%{}%\' and status = 0".format(
            WechatName)
    ...
    def execute(self, cursor: pymysql.cursors.Cursor):
        try:
            ...
            if len(exam_id) >= 1:
                ...
                exam_id = exam_id[0][0]
                cursor.execute(
                    "select count(*) from cmf_tp_correct as a join cmf_tp_subject as b on a.p_id = b.id join cmf_tp_test_paper as c on b.p_id = c.id where c.p_id = {} and a.user_id = {} and a.status = 1".format(
                        exam_id, self.TeacherId))
                returned = cursor.fetchall()
                return returned[0][
                    0], teacher_type  # This is an int number, and an int number. The first int is number of problems not viewed, the second is the teacher type.
            else:
